...
function getEventAbi(name: 'TransferOpened' | 'TransferReleased' | 'TransferRefunded') {
...
export function isTransferOpenedEvent(log: Log): boolean {
  return log.topics[0] === TRANSFER_OPENED_SIG;
}

export function isTransferReleasedEvent(log: Log): boolean {
  return log.topics[0] === TRANSFER_RELEASED_SIG;
}

export function isTransferRefundedEvent(log: Log): boolean {
...
export function decodeTransferOpened(log: Log): TransferOpenedDecoded {
  ...
  const args = raw.args as unknown as { transferId: `0x${string}`; bank: `0x${string}`; beneficiary: `0x${string}`; amount: bigint; commitment: `0x${string}`; expiry: bigint };
...
export function decodeTransferReleased(log: Log): TransferReleasedDecoded {
  ...
  const args = raw.args as unknown as { transferId: `0x${string}`; to: `0x${string}`; amount: bigint };
...
export function decodeTransferRefunded(log: Log): TransferRefundedDecoded {
